...
public class BoardListController implements Controller {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response)
		...
		int dept_num = Integer.parseInt(mb.getDept_num());
		int intPage = 1;
		...
		try {
			...
			if ("notice".equals(mode)) {
				...
				/*
				 * System.out.println("현재페이지:"+intPage); System.out.println("총페이지:"+totalPage);
				 * System.out.println("시작페이지:"+startPage); System.out.println("끝페이지:"+endPage);
				 */
				...
				totalCount = service.findCount(dept_num);
				// 총페이지수계산
				int totalPage = 0;
				...
				List<DepartBoard> list = service.findAll(intPage, dept_num);
				PageBean<DepartBoard> pb = new PageBean<>();
				pb.setCurrentPage(intPage);// 현재페이지
				pb.setTotalPage(totalPage); // 총페이지
				pb.setList(list); // 목록
				pb.setStartPage(startPage); // 시작페이지
				...
				if (flag == 0) {
					...
					forwardURL = "/board/deptboardresult.jsp";
				}
