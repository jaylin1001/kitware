			...
			<table class="table table-bordered">
					...
					<td colspan="5">
						<div class="replace">
							<select name="dept" class="dept">
								...
								<option id="init" value="0" selected>사원 선택</option>
								<c:forEach var="emp" items="${requestScope.memberlist }">
									<option id="${emp.dept_num }${emp.position_num}"
										style="display: none" value="${emp.emp_num}" name="replace">${emp.name}</option>
								</c:forEach>
							</select>
						</div>
					</td>
				...
				<tr>
					<td colspan="6" align="center">
						<button id="go">제출</button> <!-- <input type="submit" class="btn btn-success" value="제출"> -->
						<input type="button" value="취소"
						onclick="window.location.href='${pageContext.request.contextPath}/doclist.do'">
					</td>
